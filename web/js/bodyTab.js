...
layui.define(["element","jquery"],function(exports){
    ...
    //
    //function clickInc(title) {
    //    //window.location.href = "../page/recinfo/recinfoHabitInc.aspx?type=" + title;
    //    window.open("../page/recinfo/recinfoHabitInc.aspx?type=" + title, "_blank");
    //}
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
                    ...
                    //content: "<iframe src='" + _this.attr("data-url") + "' id='" + tabIdIndex + "' data-id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
				...
				window.sessionStorage.setItem("curmenu",JSON.stringify(curmenu));  //当前的窗口
                element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
                //var tmpindex = _this.attr("data-id");
                //tmpindex.window.location.reload();
                //记录业务标签点击
                //clickInc(_this.find("cite").text());
                that.tabMove(); //顶部窗口是否可滚动
