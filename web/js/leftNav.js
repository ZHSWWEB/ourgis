	...
	for(var i=0;i<data.length;i++){
		...
		if(data[i].children != undefined && data[i].children.length > 0){
			...
			for(var j=0;j<data[i].children.length;j++){
                ...
                //ulHtml += '<cite onclick="tabReload(\'' + data[i].children[j].href + '\')">'+data[i].children[j].title+'</cite></a></dd>';
...
function tabReload(childrenpageurl) {
    //window.open(childrenpageurl);
    window.location.href = childrenpageurl;
}
