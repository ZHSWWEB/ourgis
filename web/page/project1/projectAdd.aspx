...
<body class="childrenBody">
    <form id="form1" class="layui-form" style="width:48%;float:left">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button type="button" class="layui-btn" onclick="projectSave()">立即提交</button>
    ...
    <script>
        ...
        function projectTimer() {
            if (maxt == 0)
            {
                //clearTimeout(t);
                maxt = 10;
                alert("保存项目超时，请重新创建项目")
                return;
            }

            if (document.getElementById("pointSaved").getAttribute("data-value").toString() == "1") {
                //clearTimeout(t);
                maxt = 10;
            ...
            {
                maxt--;
                t = setTimeout("projectTimer()", 500);
            }
        }

        function projectSave() {
            var pointReady = document.getElementById("pointReady").getAttribute("data-value").toString();
            if (pointReady != "1") {
                alert("请先选定项目位置")
            } else {
