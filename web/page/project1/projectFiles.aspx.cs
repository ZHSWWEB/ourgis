...
namespace web.page.project
{
    public partial class projectFiles : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //判断能否上传
            bool canload = false;
            string sqlstr = "SELECT PROJECT FROM PERMISSION WHERE USERID = " + Request.Cookies["userId"].Value;
            ...
            if (canload == false)
            {
                ...
                //loadupload.Cells.Clear();
                //HtmlTableCell htc = new HtmlTableCell("td");
                //htc.InnerHtml = ;
            }
            //ProjectFilesLoad(filetype, prosystemid);
            listPageDiv.InnerHtml = "<iframe id=\"listfilepage\" frameborder=\"0\" width=\"100%\" height=\"600px\" src=\"./projectFilesList.aspx?proid=" + prosystemid + "&tabtype=" + filetype + "\" onload=\"LoadHeight(this)\">抱歉！该页面不支持您的浏览器！</iframe>";
        }
        ...
        public static string FileType2TableName(string type)
        {
        ...
        public static string FileType2ColumnName(string type)
        {
            ...
            {
                //可行性研究报告
                columnname = "FEASIBILITY_STUDY";
            }
            else if (type == "3")
            {
                //初步设计
                columnname = "PRELIMINARY_DESIGN";
            }
            else if (type == "4")
            {
                //施工图及招标书
                columnname = "DETAILED_DESIGN";
            }
            else if (type == "5")
            {
                //计划变更报告
                columnname = "DESIGN_CHANGE";
            }
            else if (type == "6")
            {
                //计算书
        ...
        protected void btnSaveFile_Click(object sender, EventArgs e)
        {
            if (FileUpload1.HasFile)
            {
                ...
                if (prods.Tables[0].Rows.Count > 0)
                {
                    ...
                    //修改项目中对应资料的有无状态
                    sqlstr = "UPDATE PROJECT_INFO SET " + FileType2ColumnName(filetype) + " = '有' WHERE SYSTEMID = " + proid;
                    OperateSde(sqlstr);
